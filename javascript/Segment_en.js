    ...
    var b = 0
      ...
      , d = {
        ...
        asHTML: function(b) {
            ...
            res.attr("src", ``),
            res.attr("usemap", "#image-map");

            var maparea = a("<map />");
            maparea.attr("name", "image-map");

            var g = [];
            a.each(c[b].area, function(res, e) {
                var maparea = [];
                if (a.each(e.coords, function(a, b) {
                    maparea.push(b.naturalX, b.naturalY)
                }),
                "circle" == e.shape) {
                    var h = Math.round(Math.sqrt(Math.pow(maparea[2] - maparea[0], 2) + Math.pow(maparea[3] - maparea[1], 2)));
                    maparea = maparea.slice(0, 2),
                    maparea.push(h)
                }
                var i = a("<area />");
                i.attr("target", c[b].area[res].target),
                i.attr("alt", c[b].area[res].title),
                i.attr("title", c[b].area[res].title),
                i.attr("href", c[b].area[res].href),
                i.attr("coords", maparea.join(",")),
                i.attr("shape", c[b].area[res].shape),
                g.push(i[0].outerHTML)
                console.log("g-->",g);
            }),
            maparea.append("\n    " + g.join("\n    ") + "\n");
            var headcomment = "<!--Generated Result-->";
...
function(a) {
    a(document).on("init", function() {
        ...
        a(document).on("click", "#modal-code", function() {
            ...
            var coordsMatch = b.match(/coords="([\d\s,]+)"/);

            if (coordsMatch) {
                var coordsString = coordsMatch[1];
                ...
                // Tạo văn bản tọa độ
                var coordinateText = '<span style="display:inline-block;">Coordinates: </span><span style="display:inline-block;">&nbsp;x<sub>1</sub>(' + x1 + ', ' + y1 + '), x<sub>2</sub>(' + x2 + ', ' + y2 + ')</span>';
            
                // Thêm văn bản tọa độ vào thẻ có class "coordinate-test"
                var coordinateElement = document.getElementById('coordinate-text');
                if (coordinateElement) {
                    coordinateElement.innerHTML = coordinateText;
                } else {
                    // console.log("Không tìm thấy thẻ có id là 'coordinate-text'.");
                    createToast('error', 'fa-solid fa-circle-exclamation', 'Error', 'Could not connect to Server');
                }
            } else {
                // console.log("Không tìm thấy chuỗi coords trong mã HTML.");
                createToast('error', 'fa-solid fa-circle-exclamation', 'Error', 'Could not connect to Server');
            }
            ...
            if (imageSizeElement) {
                imageSizeElement.textContent = image_size;
            } else {
